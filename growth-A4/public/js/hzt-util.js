...
function checkToken(url, callback, method, userData, dataType) {
    ...
    if(!hztInfo.hasOwnProperty("authInfo") || !hztInfo.hasOwnProperty("headerVersion")) {
        callNative("auth", 1, JSON.stringify({}), function (params) {
            ...
            request(url, successCallback, method, userData, errorCallback);
        });
    } else {
        request(url, successCallback, method, userData, errorCallback);
    }
...
function request(url, callback, method, userData, errorCallback) {
    var req = {
        url: url,
        type: method,
        dataType: "text",
